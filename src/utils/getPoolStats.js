...
const fetchPoolPrices = async (
  ...
  const knownTokenValuation = knownTokenBalance
    .div(knownToken.decimals)
    .times(knownTokenPricePerUnit);

  const unknownTokenBalance = await fetchPoolTokenBalance(
    lp.address,
    unknownToken.address,
    chainId
  );
  const unknownTokenValuation = knownTokenValuation;
  const unknownTokenPriceUnit = unknownTokenValuation.div(
    unknownTokenBalance.div(unknownToken.decimals)
  );
...
const fetchAmmPoolsPrices = async (pools, knownPrices) => {
  ...
  for (const pool of [...pools].reverse()) {
    ...
    if (
      tokenPrices.hasOwnProperty(pool.lp0.oracleId) ||
      tokenPrices.hasOwnProperty(pool.lp1.oracleId)
    ) {
